...
def compute_lattice_scores_from_ratemaps_2d(scorers: List[GridScorer],
                                            ratemaps_2d: np.ndarray,
                                            n_recurr_units_to_analyze: int = 128,
                                            ) -> Dict[int, Dict[str, np.ndarray]]:

    # Choose random subset of neurons to analyze.
    neuron_indices = np.random.choice(len(ratemaps_2d),
                                      replace=False,
...
def compute_periodicity_and_orientation_from_ratemaps_2d(scorers: List[GridScorer],
                                                         ratemaps_2d: np.ndarray,
    ...
    for scorer in scorers:
        ...
        for lattice_type in ['60']:  # TODO: check whether period scoring works for 90d as well.
            ...
            possible_grid_cell_indices = lattice_scores[f'score_{lattice_type}_by_neuron'] > threshold
...
def create_grid_scorers(left: float,
                        right: float,
                        top: float,
                        bottom: float,
    ...
    scorers = [GridScorer(nbins=nbins,
