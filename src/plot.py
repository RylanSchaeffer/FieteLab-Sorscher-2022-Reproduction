...
def plot_lattice_scores_by_nbins(lattice_scores_by_nbins_dict: Dict[int, Dict[str, np.ndarray]],
    ...
    for col_idx, (nbins, lattice_scores) in enumerate(lattice_scores_by_nbins_dict.items()):
        ax = axes[col_idx]
...
def plot_ratemaps_2d(ratemaps: np.ndarray,
                     wandb_logger = None,
    ...
    for unit_idx, ratemap in enumerate(ratemaps):
        ...
        if smooth_ratemap_before_plotting:
            ...
            ratemap = scipy.ndimage.gaussian_filter(ratemap, sigma=2.)
        ...
        if extreme_coords is not None:
            if col == 0:
                ax.set_ylabel('({:.1f}, {:.1f})'.format(
                    extreme_coords['bottom'],
                    extreme_coords['top'],
                ))
            if row == 0:
                ax.set_xlabel('({:.1f}, {:.1f})'.format(
                    extreme_coords['left'],
